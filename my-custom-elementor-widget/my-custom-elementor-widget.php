    ...
    if ( ! class_exists( 'Elementor_My_Custom_Widget' ) ) {
        class Elementor_My_Custom_Widget extends \Elementor\Widget_Base {
            ...
            public function get_keywords() {
                return [ 'custom', 'repeater', 'image', 'description', 'single file' ];
            }
            ...
            protected function render() {
                ...
                if ( $settings['my_repeater_list'] ) {
                    ...
                    foreach ( $settings['my_repeater_list'] as $item ) {
                        ...
                        echo '<div class="my-custom-widget-item elementor-repeater-item-' . esc_attr( $item_id ) . '">';
                        ...
                        if ( ! empty( $item['item_image']['url'] ) ) {
                            ...
                            if ( $image_id ) {
                                echo wp_get_attachment_image( $image_id, $image_size, false, [ 'class' => 'my-custom-widget-image' ] );
                            } else {
                                // Fallback for external URLs or if image ID is not set
                                echo '<img src="' . esc_url( $item['item_image']['url'] ) . '" alt="' . esc_attr( $item['item_title'] ) . '" class="my-custom-widget-image" />';
                            }
                        }

                        // Render title
                        if ( ! empty( $item['item_title'] ) ) {
                            echo '<h3 class="my-custom-widget-title">' . esc_html( $item['item_title'] ) . '</h3>';
                        }

                        // Render description
                        if ( ! empty( $item['item_description'] ) ) {
                            echo '<div class="my-custom-widget-description">' . wp_kses_post( $item['item_description'] ) . '</div>';
                        }

                        echo '</div>'; // .my-custom-widget-item
                    }
                    echo '</div>'; // .my-custom-widget-wrapper
                }
            }
        }
    }
